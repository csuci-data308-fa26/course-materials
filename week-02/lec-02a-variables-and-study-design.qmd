...
subtitle: "Variables and study design"
...
#| fig-alt: "Diagram showing 6 steps that flow linearly into one another, from 'Ask' to 'Design' to 'Collect' to 'Summarise' to 'Analyse' to 'Report'."
...
- What variables are involved and what type are they?
- How could we collect data to investigate this?
...
#| echo: false
#| fig-align: center
#| fig-alt: "Repeat diagram from previous slide showing the letters 'X' and 'Y', with an arrow pointing from 'X' to 'Y' with a question mark above."
#| out-width: "50%"
knitr::include_graphics("img/diagram-causal.png") 
...
```{r}
#| label: diag-confounding-ex-unknown
#| echo: false
#| fig-align: center
#| fig-alt: "Diagram showing the phrase 'Exercising' with an arrow pointing to the word 'GPA'. There are also arrows point to both of these variables from the symbol '?'."
#| out-width: "50%"
diagram <- DiagrammeR::grViz("
    digraph {
        # graph aesthetics
...
tmp <- capture.output(rsvg::rsvg_png(
  charToRaw(DiagrammeRsvg::export_svg(diagram)),
  file = "img/diagram-confounding-example-unknown.png",
  height = 250
))

knitr::include_graphics("img/diagram-confounding-example-unknown.png") 
...
#| fig-out: 80%
#| fig-cap: "Image from <https://bookdown.org/pkaldunn/SRM-Textbook/Intro.html>"
#| fig-alt: "Repeate diagram of research process showing 6 steps that flow linearly into one another, from 'Ask' to 'Design' to 'Collect' to 'Summarise' to 'Analyse' to 'Report'."
knitr::include_graphics("img/research-steps.png")
```
...
- Causal relationships between variables cannot be established from a single observational study because of the possibility of confounding
...
- In **quasi-experiments**, researchers do not separate the cases into groups, but they do randomly assign treatment to each group.

## Experimental design video

Starting at 3 minutes:
[Video on random assignment](https://youtu.be/4s_QTi2AJ4k?si=z21fGGOWkypJCRin)
...
A/B testing is terminology used in tech when you want to compare two methods

- E.g., Comparing two web designs to see which has a higher user engagement

- Fancy sounding wording for a randomized experiment with a binary explanatory variable
...
- Can use **matching** to match on variables you think may be confounding between cases in both groups

In a **within-subjects** experiment each case experiences all levels of the treatment

- Need to alternate how treatment is experiences so order does not affect the response
...
- Exploratory research can often lead to confirmatory research, but those would be different studies!

## Cross-sectional versus longitudinal

In **cross-sectional** studies the data is collected at one point in time 

In **longitudinal** studies there are multiple observations on each participant / case  from multiple points in time

## Cough drop study

Let's discuss the assigned [research article](https://www.jabfm.org/content/jabfp/31/2/183.full.pdf) which investigated cough drops.

- What was their research question?
- What was their response and explanatory variables?
- What was their study design?
- Was it exploratory or confirmatory research?
- Was a causal conclusion possible?
